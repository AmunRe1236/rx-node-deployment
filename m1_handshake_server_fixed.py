...
"""
GENTLEMAN M1 Handshake Server - Fixed Version
Zentrale Koordination für GENTLEMAN Cluster
"""
...
class HandshakeHandler(BaseHTTPRequestHandler):
    ...
    def _run_ssh_command(self, cmd, timeout=8):
        """Führe SSH-Befehl mit Timeout aus"""
        try:
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=timeout)
            return result
        except subprocess.TimeoutExpired:
            logging.warning(f"⚠️ SSH Timeout nach {timeout}s: {' '.join(cmd)}")
            return None
        except Exception as e:
            logging.error(f"❌ SSH Fehler: {e}")
            return None
    
    def handle_rx_node_status(self):
        """Handle RX Node Status Check Request - Fixed Version"""
        try:
            source = "unknown"
            ...
            # SSH-Befehl mit kurzen Timeouts
            ssh_cmd = [
                'ssh', '-i', ssh_key, '-o', 'StrictHostKeyChecking=no',
                '-o', 'ConnectTimeout=3', '-o', 'ServerAliveInterval=2',
                '-o', 'ServerAliveCountMax=1', f'amo9n11@{rx_node_ip}',
                'uptime && hostname && whoami'
            ]
            
            # Führe SSH-Befehl mit ThreadPoolExecutor und Timeout aus
            try:
                future = self.executor.submit(self._run_ssh_command, ssh_cmd, 5)
                ...
                if result and result.returncode == 0:
                    ...
                    status = "offline"
                    message = f"RX Node SSH Fehler: {result.stderr if result else 'Timeout'}"
                    details = {
                        'ssh_accessible': False,
                        'error': result.stderr if result else 'SSH Timeout'
                    }
    ...
    def handle_rx_node_shutdown(self):
        ...
        try:
            ...
            try:
                future = self.executor.submit(self._run_ssh_command, ssh_cmd, 8)
                ...
                if result and result.returncode == 0:
                    ...
                    logging.error(f"❌ RX Node Shutdown Fehler: {result.stderr if result else 'Timeout'}")
                    status = "error"
                    message = f"SSH Fehler: {result.stderr if result else 'Timeout'}"
    ...
    def handle_bootup_request(self):
        ...
        try:
            ...
            target_mac = request_data.get('target_mac', '00:00:00:00:00:00')
...
class HandshakeServer:
    ...
    def start(self):
        ...
        try:
            ...
            logging.info("📡 Endpoints verfügbar:")
            logging.info("   POST /handshake               - Node Registrierung")
            logging.info("   GET  /status                  - Cluster Status")
            logging.info("   GET  /nodes                   - Aktive Nodes")
            logging.info("   GET  /health                  - Health Check")
            logging.info("   POST /admin/shutdown          - M1 Mac Remote Shutdown")
            logging.info("   POST /admin/bootup            - M1 Mac Remote Bootup (Wake-on-LAN)")
            logging.info("   POST /admin/rx-node/shutdown  - RX Node Remote Shutdown")
            logging.info("   POST /admin/rx-node/wakeup    - RX Node Wake-on-LAN")
            logging.info("   GET  /admin/rx-node/status    - RX Node Status Check")
            ...
            logging.info("🛑 Server gestoppt durch Benutzer")
        except Exception as e:
    ...
    def status_monitor(self):
        ...
        while True:
            try:
                ...
                logging.info(f"📊 Status: {len(active_nodes)} aktive Nodes")
            except Exception as e:
...
def main():
    ...
    server = HandshakeServer(args.host, args.port)
    server.start()

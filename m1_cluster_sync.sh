...
# 🍎 M1 Mac GENTLEMAN Cluster SSH Synchronisation
...
CLUSTER_SCRIPT="cluster_ssh_rotation.sh"
...
log_header() {
    echo -e "${PURPLE}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${PURPLE}║  🍎 M1 MAC CLUSTER SSH SYNCHRONISATION                       ║${NC}"
...
analyze_m1_keys() {
    ...
    for key_file in "$M1_SSH_DIR"/*; do
        if [[ -f "$key_file" && "$key_file" =~ \.(pub|key)$ ]]; then
            ((key_count++))
...
test_cluster_connectivity() {
    ...
    local total_count=${#CLUSTER_NODES[@]}
    
    for node_name in "${!CLUSTER_NODES[@]}"; do
        local node_address="${CLUSTER_NODES[$node_name]}"
...
- **Anzahl Backups**: $(ls -1 "$M1_BACKUP_DIR"/gentleman_key_* 2>/dev/null | wc -l)
...
$(if [[ $(ls -1 "$M1_BACKUP_DIR"/gentleman_key_* 2>/dev/null | wc -l) -gt 15 ]]; then
...
**Report erstellt von**: M1 Mac Cluster Sync Script v1.0
EOF
...
case "${1:-sync}" in
    ...
    "report")
        ...
        ;;
    "help"|"-h"|"--help")
        echo "🍎 M1 Mac GENTLEMAN Cluster SSH Synchronisation"

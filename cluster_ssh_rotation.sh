...
# 🔄 GENTLEMAN Cluster-weite SSH Key Rotation & Synchronisation
# Version: 2.0
...
CLUSTER_NODES["i7-node"]="amonbaumgartner@192.168.68.105:client"
CLUSTER_NODES["rx-node"]="amo9n11@192.168.68.117:primary_trainer"
CLUSTER_NODES["m1-mac"]="amonbaumgartner@192.168.68.111:coordinator"
...
log_header() {
    echo -e "${PURPLE}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${PURPLE}║  🔄 GENTLEMAN CLUSTER SSH KEY ROTATION & SYNC                ║${NC}"
...
test_node_connectivity() {
    ...
    local node_address="${node_info%:*}"
    local node_role="${node_info#*:}"
...
discover_cluster() {
    ...
    # Export status for other functions
    for node_name in "${!NODE_STATUS[@]}"; do
        export "NODE_${node_name//-/_}_STATUS=${NODE_STATUS[$node_name]}"
    done
}
...
distribute_cluster_key() {
    ...
    for node_name in "${!CLUSTER_NODES[@]}"; do
        ...
        local node_info="${CLUSTER_NODES[$node_name]}"
        local node_address="${node_info%:*}"
        local node_status_var="NODE_${node_name//-/_}_STATUS"
        local node_status="${!node_status_var:-unknown}"
...
update_cluster_config() {
    ...
    if command -v jq &>/dev/null; then
        jq --arg timestamp "$TIMESTAMP" \
           '.cluster_rotation.last_rotation = $timestamp' \
...
cleanup_old_backups() {
    ...
    local backup_count=$(ls -1 "$BACKUP_DIR"/gentleman_key_* 2>/dev/null | wc -l)
...
case "${1:-run}" in
    ...
    "cleanup")
        ...
        ;;
    "help"|"-h"|"--help")
        echo "🔄 GENTLEMAN Cluster SSH Key Rotation"
